...
public class CalibrationView : MonoBehaviour
{
    ...
    private struct CalibrationPoint
    {
        ...
        [Tooltip("The angle of rotation around the Z axis (forward direction) of the camera, with 0 degrees to the right and 180 degrees to the left.")]
...
#pragma warning disable 0649 // Field is never assigned to, and will always have its default value null
    [SerializeField, Tooltip("The number of calibration points and their pattern. Points are positioned at 2 metres from the camera, and the angle from center and the angular offset around center (counter-clockwise, starting from right hand side) is customizable.")]
    ...
    [SerializeField, Tooltip("The animation curve of the point movement; controls acceleration of the point movement.")]
...
#pragma warning restore 0649
    ...
    /// </summary>
    private const string InstructionsMessageText = "Follow the blue dot"; // The instruction text at the start of a calibration.
    private const string CalibrationSuccessfulMessageText = "Calibration successful!"; // The text to present when calibration succeeds.
    private const string CalibrationErrorFailedToComputeMessageText = "Calibration failed to compute model"; // The text to present when calibration fails to compute the result..
    private const string CalibrationErrorUnknownMessageText = "Calibration failed. Unknown error"; // The text to present when calibration fails with unkown error.
    private const float DisplayInstructionsDuration = 3.0f; // The duration of the instructions text.
    private const float UserGettingReadyDuration = 1.0f; // The duration between the end of the instructions text and the start of the movement of the first calibration point.
    private const float StimuliMinimumDuration = 0.8f; // The minimum duration for displaying a calibration point. Collection time per stimuli point varies due to various factors. Set to 0 if you want the point to move as soon as the collection is done. This is mainly for UX reasons to allow the user to follow the calibration without stress.
    private const float DisplayResultsDuration = 3.0f; // The duration for displaying the results text.
    private const float TimeDelayBeforeCollectingData = 0.5f; // Time delay after the point stops moving and before the data collection begins.
    ...
    private IEnumerator MoveCalibrationPoint()
    {
        ...
        var rotationAngleAroundCameraForwardVector = calibrationPattern[_currentPointIndex].angularOffsetAroundCenter;
